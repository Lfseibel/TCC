  ...
  <div class="flex items-center">
    ...
    @if (auth()->user()->type === 'Admin')
      <form id="{{$date}}" action="{{ route('reservationdate.destroy', [$reservation->code, $date]) }}" method="POST">
        @method('DELETE')
        @csrf
        <button type="button" class="rounded-full bg-red-500 hover:bg-red-700 text-white delete-button"><svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-6 h-6 ">
...
@endforeach
  ...
  <form id="{{$reservation->code}}" action="{{ route('reservation.destroy', $reservation->code) }}" method="POST">
    @method('DELETE')
    @csrf
    <button type="button" class="w-full rounded-full bg-red-500 hover:bg-red-700 text-white font-bold mt-8 py-2 px-4 delete-reservation">Deletar</button>
  </form>
@endif
